...
/**
 * OpenID provider, copied from Zend_OpenId_Provider as that class is using private methods, making extending the class pointless.
 */
class XenSSO_Master_OpenID_Provider
{
	 ...
	 */
	public function __construct($loginUrl, $trustUrl)
	{
		$this->_storage 	= new XenSSO_Master_OpenID_Storage();
		$this->_loginUrl 	= Zend_OpenId::absoluteUrl($loginUrl);
		$this->_trustUrl 	= Zend_OpenId::absoluteUrl($trustUrl);
		$this->_user 		= new Zend_OpenId_Provider_User_Session();
		$this->_sessionTtl 	= 3600;
	}
        ...
        if (isset($params['openid_mode'])) {
            if ($params['openid_mode'] == 'associate') {
                ...
                if (!empty($params['openid_return_to'])) {
                    $this->redirect($params['openid_return_to'], $ret, $response);
                }
                return true;
        ...
        Zend_Controller_Response_Abstract $response = null)
    {
        ...
        if (!isset($params['openid_identity']) ||
            ...
            if ($immediate) {
                ...
                $this->redirect($this->_loginUrl, $params2, $response);
                return true;
            }
        }
        ...
        if ($trusted === false) {
            ...
            if ($immediate) {
                ...
                $this->redirect($this->_trustUrl, $params2, $response);
                return true;
            }
        }
